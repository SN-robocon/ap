...
//   left_wheel_(EPort::PORT_B,spikeapi::Motor::EDirection::COUNTERCLOCKWISE,true), 
//   right_wheel_(EPort::PORT_A,spikeapi::Motor::EDirection::CLOCKWISE,true),  
...
void PidController::PidControl() {
  const float kp = 0.25;    //比例ゲイン(1より小さい値で検討)
  const float ki = 0.01;   //積分ゲイン
  const float kd = 0.002;     //微分ゲイン
  

  const int target = 12;    //目標値
