...
void SmartCarryDrive::DriveSwitch(int scene_num) {
	...
	switch (scene_num) {
		case 1:  // ライントレース
            line_trace_drive_l_.Drive(ControlMode::LINE_TRACE_R,60);
		break;
		...
		case 12:  //旋回(左に90度)
            turn_drive_.Drive(ControlMode::TURN_RIGHT,80);
		break;
		...
		case 14:  // 旋回
			turn_drive_.Drive(ControlMode::TURN_RIGHT,70);//80
		break;
		...
		//通常通り
		case 16:  // ライントレース
            line_trace_drive_r_.Drive(ControlMode::LINE_TRACE_L,60);
		break;
		...
		case 23:  //旋回(右に45度)
			turn_drive_.Drive(ControlMode::TURN_LEFT,80);  //直進(黒ライン検知)
		break;
